...
class NetworkMonitor(app_manager.RyuApp):
    '''
    获得网络中流的统计信息以及端口的统计信息
    在此基础上得到链路带宽的消耗量以及链路可用带宽
    '''
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1], key=lambda flow: (flow.match.get('in_port'),flow.match.get('ipv4_dst'))):
            key = (stat.match['in_port'], stat.match.get('ipv4_dst'),stat.instructions[0].actions[0].port)
